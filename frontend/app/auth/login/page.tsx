...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await startAuditAndGo(data.access_token);
    } catch (err: unknown) {
  ...
  const registerHref = pendingUrl
    ? `/auth/register?url=${encodeURIComponent(pendingUrl)}`
    : "/auth/register";
      ...
      <div className="w-full max-w-[420px] bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
        ...
        {pendingUrl && (
          <p className="mb-6 text-center text-sm text-gray-500">
            Après connexion, audit gratuit pour{" "}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input
              ...
              required
              className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition"
