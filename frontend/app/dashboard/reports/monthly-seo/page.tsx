...
export default function MonthlySeoReportPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Combined GSC + GA4 performance for {monthLabel}
          </p>
        </div>
      ...
      {sites.length === 0 ? (
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-8 space-y-10">
          {/* Report header */}
          ...
          <section>
            <h3 className="text-sm font-semibold text-gray-900 uppercase tracking-wide mb-4">
              Analytics Overview (GA4)
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
              <div>
                <p className="text-xs text-gray-500 mb-1">Sessions</p>
                <p className="text-2xl font-bold text-violet-600">
                  {formatNumber(report.ga4_summary.total_sessions)}
                </p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Users</p>
                <p className="text-2xl font-bold text-violet-600">
                  {formatNumber(report.ga4_summary.total_users)}
                </p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Pageviews</p>
                <p className="text-2xl font-bold text-violet-600">
                  {formatNumber(report.ga4_summary.total_pageviews)}
                </p>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <table className="w-full text-sm">
              <thead>
  <tr className="text-xs text-gray-600 font-semibold border-b border-gray-200 bg-gray-50">
    <th className="text-left py-2.5 px-1">Query</th>
    <th className="text-right py-2.5 px-1">Clicks</th>
    <th className="text-right py-2.5 px-1">Average Position</th>
  </tr>
              </thead>
              <tbody>
      ...
      <tr key={kw.keyword} className="border-b border-gray-100 hover:bg-gray-50">
        ...
        <td className="py-2.5 text-right text-gray-800">
          ...
          <p className="text-xs text-gray-400 pt-4 border-t border-gray-100">
            Report data is taken from Google Search Console & Analytics via
            NexRank.
          </p>
        </div>
      )}
